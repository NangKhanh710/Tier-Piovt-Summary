...
.stButton > button:hover { background: #7b2ff7 !important; }
...
TIER_COLORS     = {"Tier 1": "#00b894", "Tier 2": "#6c5ce7", "Tier 3": "#0984e3", "Tail-end": "#d63031"}
...
def to_excel_bytes(pivot_df: pd.DataFrame) -> bytes:
    EXCEL_TIER_COLORS = {
        "Tier 1": "FF00B894", "Tier 2": "FF6C5CE7",
        "Tier 3": "FF0984E3", "Tail-end": "FFD63031", "Total": "FF1A0050"
    }
    output = BytesIO()
    ...
    for cell in ws[1]:
        cell.fill      = PatternFill("solid", fgColor="FF1A0050")
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        for cell in row:
            ...
            if cell.column == 1:
                ...
                cell.fill = PatternFill("solid", fgColor="FFFFFFFF" if not is_total else "FFF0EEFF")
                cell.font = Font(color="FF1A1A2E" if not is_total else "FF1A0050",
...
display_tier = tier_df.copy() #Frozen Tier table 
...
st.dataframe(
    pivot_df.style.format("{:.3f}")
        .background_gradient(cmap="Blues", axis=1, subset=pd.IndexSlice[:"Tail-end", :]),
    use_container_width=True,
    height=230,
)
...
# Chart 1: Monthly revenue by Tier (line chart)
with chart_col1:
...
with chart_col3:
    ...
    count_data["Color"] = count_data["Tier"].map(TIER_COLORS)
    fig3 = px.bar(
...
# Chart 4: Cumulative revenue waterfall (top 20 accounts)
with chart_col4:
    st.markdown("**Top 20 Accounts — Cumulative Revenue %**")
    top20 = tier_df.head(20).copy()
    top20["CumPct"] = (top20["Cumulative %"].str.replace("%","").astype(float)
                       if top20["Cumulative %"].dtype == object
                       else top20["Cumulative %"] * 100)
    # recompute from raw
    top20 = tier_df.head(20).copy()
    ...
    fig4.add_trace(go.Bar(
        ...
        y=top20["% Grand Total"] * 100 if top20["% Grand Total"].max() <= 1 else top20["% Grand Total"],
